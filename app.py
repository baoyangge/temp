...
# ==============================================================================
# AI回答後の自動スクロール無効化（scrollTopインターセプター） #20260414_李修正
# ==============================================================================
components.html(
    """
    <script>
        // Streamlitの親フレーム（実際のページ）のプロパティをインターセプトする #20260414_李修正
        const parentWindow = window.parent;
        if (!parentWindow.__scrollTopIntercepted) {
            // Element.prototype にある本来の scrollTop の定義（getter/setter）を取得
            const originalDescriptor = Object.getOwnPropertyDescriptor(parentWindow.Element.prototype, 'scrollTop');
            
            if (originalDescriptor && originalDescriptor.set) {
                // scrollTop プロパティを再定義してハイジャックする
                Object.defineProperty(parentWindow.Element.prototype, 'scrollTop', {
                    get: function() {
                        return originalDescriptor.get.call(this);
                    },
                    set: function(value) {
                        // 対象がチャットコンテナ（st.container等）であるかどうかを判定
                        if (this.getAttribute) {
                            const testId = this.getAttribute('data-testid');
                            // Streamlit 1.56.0 ではチャットのスクロールコンテナがこれらのテストIDを持つことが多い
                            if (testId === 'stVerticalBlock' || testId === 'stVirtualScrollContainer' || testId === 'stMainBlockContainer') {
                                // プログラムからの scrollTop 代入（自動スクロール）を無視する
                                // ※ ユーザーのマウスやトラックパッドでの手動スクロールは、この setter を経由しないため影響を受けません
                                return; 
                            }
                            // className に特定の文字列が含まれる場合もブロック
                            if (this.className && typeof this.className === 'string' && this.className.includes('stVerticalBlock')) {
                                return;
                            }
                        }

                        // 条件に当てはまらない通常のスクロールは放行（元の setter を呼び出す）
                        return originalDescriptor.set.call(this, value);
                    }
                });
                parentWindow.__scrollTopIntercepted = true;
            }
        }
    </script>
    """,
