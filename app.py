...
下記コードを以下の点を機能追加してください：
右下の「全プラン詳細確認」のボタン下に、調整できる詳細表を追加してください。
表示すべきデータは、レーダチャットの現在選択しているプランの特約保険料金額確認の付加内容。
例えば、現在レーダチャットは松プランを選択している場合は、松プランの特約分類（略称も可能）-特約名-付加内容を表示すること（保険料は要らない）
また、金額の左側右側に、"-"と"+"があって、すぐデータ調整できるようにしたいです(数字部分は手入力も可能、付加/なしは-+で切り替える)。
項目は多いので、この部分スクロールできるようにください

下記は例です：
[全プラン詳細確認]
病気（病気・ケガへの備えの略称）　入院治療保障特約　-付加+
上記病気と同じ大き枠（色もあり）　⼊院初期⼀時⾦給付特約		-50000+
上記病気と同じ大き枠	新・入院特約	-5000+
　
